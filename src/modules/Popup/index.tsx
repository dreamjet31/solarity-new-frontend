...
const Library = (props) => {
    const { setPage, selectGame, createEventToggle, setCreateEventToggle } =
        props
    ...
    return (
        <div className="flex flex-col w-full px-[30px]">
            ...
            <div className="gap-[32px] grid custom-2xl:grid-cols-6 xl:grid-cols-5 lg:grid-cols-4 md:grid-cols-3 sm:grid-cols-2 xs:grid-cols-1 justify-items-center my-[16px]">
                {eventTabIndex === 0 && eventsList
                    ? eventsList
                          .filter(
                              (event, index) => event.type === activeEventsMenu
                          )
                          .map((item, index) => (
                              <LiveEventPanel
                                  data={item}
                                  key={index}
                                  onClick={() => onClickEventItem(item)}
                              />
                          ))
                    : null}
                {eventTabIndex === 1 && eventsList
                    ? eventsList
                          .filter(
                              (event, index) => event.type === activeEventsMenu
                          )
