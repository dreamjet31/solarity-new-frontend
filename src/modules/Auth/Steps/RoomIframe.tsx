...
const RoomIframe = (props) => {
  ...
  const { roomInfo, profile } = useSelector((state: RootStateOrAny) => ({
  ...
  return (
    <div className="rounded-[20px] border-[1px] border-primary bg-[#1a1a1c] p-3 mt-5 lg:mt-0">
      ...
      <div className="text-[#929298] mt-3">
        ...
        {profile.rooms && profile.rooms.findIndex(room => room.roomNo == roomInfo.no) < 0 ? (
          ...
          profile.rooms && profile.rooms.findIndex(room => room.roomNo == roomInfo.no && room.active) >= 0 ? (
            <></>
          ) : (
            <button className="solarity-button font-medium py-[22px] px-[22px] rounded-[22px] text-white w-[100%] h-[52px] text-[16px] sm:text-[16px] text-center tracking-wider inline-flex items-center justify-center bg-primary" onClick={() => setActiveRoom()}>
