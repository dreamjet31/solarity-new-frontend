...
const UserDaos = (props) => {
    ...
    useEffect(() => {
        ...
        if (userInfo.solanaAddress) {
            ...
            axios
                .get(
                    `https://api.blockchainapi.com/v1/solana/wallet/mainnet-beta/${userInfo.solanaAddress}/nfts`,
                    config
                )
                .then((response) => {
                    const nfts = response.data.nfts_metadata
                    const formattedNfts = nfts.map((nft) => ({
                        name: nft.off_chain_data.collection
                            ? nft.off_chain_data.collection.name
                            : '',
    ...
    useEffect(() => {
        if (nfts.length) {
            ...
            for (let i = 0; i < collectionNfts.length; i++) {
                if (
                    daos.findIndex(
                        (dao) => dao.name === collectionNfts[i].name
                    ) < 0
                ) {
