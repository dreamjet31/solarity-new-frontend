...
const Input = (props: InputType) => {
  ...
  return (
    <div className={`w-full relative flex items-center rounded-[18px] border-[1.5px] py-1 px-2 ${classBorder}`}>
      ...
      <input className="appearance-none text-[18px] tracking-[0.02rem] bg-transparent z-50 w-full h-[48px] text-white/60 mr-3 py-1 px-2 leading-tight" onFocus={focusInput} onBlur={unFocusInput} onChange={(e) => { props.setValue(e.target.value) }} value={props.value} />
    </div>
  );
}
