  ...
  return (
    <div className="w-full">
      <div className={`relative flex items-center rounded-[18px] border-[1.5px] p-1 sm:p-2 ${classBorder}`}>
        <span className={`absolute bg-[#141416] text-[14px] sm:text-[18px] px-1 tracking-[0.02rem] z-10 ${classFocus}`}>Claim your domain</span>
        <input className="appearance-none text-[14px] sm:text-[18px] tracking-[0.02rem] bg-transparent z-50 border-r-2 border-white/10 w-full h-[48px] text-white/60 mr-3 py-1 px-2 leading-tight" onFocus={focusInput} onBlur={unFocusInput} onChange={(e) => { setDomainValue(e.target.value); changeValue(e.target.value ? (e.target.value + domainFooter) : "", "domain") }} value={domainValue} />
        <select className="block px-0 text-white bg-[#141416] border-0 peer text-[14px] sm:text-[18px]" onChange={(e) => { setDomainFooter(e.target.value); changeValue(domainValue + e.target.value) }}>
